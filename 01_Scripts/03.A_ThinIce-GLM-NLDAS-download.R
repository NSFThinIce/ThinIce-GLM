...
year<-2021 #set the year here, GL4: 2018-2024; 2018-2020: KG: B, 2020-2021 IAO: C; 2022-2024 DCR: A#### 
...
out.ts = seq.POSIXt(as.POSIXct(startdatetime, tz = loc_tz),as.POSIXct(enddatetime,tz=loc_tz), by = 'hour')
  ...
  #out.ts<-c(as.POSIXct("2021-04-21 05:00:00", tz = loc_tz),as.POSIXct("2021-01-28 18:00:00", tz = loc_tz))
...
drivers <- drivers %>% 
  rename(time = local_dateTime,
         ...
         Rain = Rain.m_day,
         Snow = Snow.m_day ) %>% 
  dplyr::select(-c(SurfPressure.Pa, SpecHumidity.kg_kg))
...
plot(drivers$time,drivers$AirTemp,type = 'l')
